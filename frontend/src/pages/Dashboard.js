  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div style={{
        ...
        <div style={{ color: '#9ca3af' }}>CI: {d?.ci_low}% – {d?.ci_high}%</div>
        <div style={{ marginTop: 4 }}>{riskChip(d?.level)}</div>
      </div>
    );
  };

  // Mobile history cards (replaces table)
  const HistoryCards = () => (
      ...
      {history.slice(0, 8).map((rec, i) => (
        <div key={i} style={{
          ...
          <div style={{ textAlign: 'center', flexShrink: 0 }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: diseaseColors[rec.disease] || '#6366f1' }}>
              {Math.round(rec.probability * 100)}%
            </div>
            <div style={{ fontSize: 10, color: '#9ca3af' }}>probability</div>
          </div>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f0f4ff' }}>
      ...
      <div style={{
        ...
        // On mobile, add bottom padding to clear the fixed bottom nav bar (64px)
        paddingBottom: isMobile ? 'calc(1rem + 64px)' : '2rem',
        overflowY: 'auto',
        minWidth: 0,
      }}>
        <h1 style={{ fontSize: isMobile ? 18 : 22, fontWeight: 700, color: '#1a1f3a' }}>
        ...
        {/* Stat cards — 3 cols → 1 col on mobile */}
        <div style={{
          display: 'grid',
          ...
          {[
            ...
            <div key={i} style={{
              ...
              <div style={{ fontSize: 12, color: s.color, fontWeight: 600, marginBottom: isMobile ? 0 : 8 }}>
                {s.label}
              </div>
              <div style={{ fontSize: isMobile ? 24 : 32, fontWeight: 700, color: s.color }}>
                {s.value}
              </div>
            </div>
          ))}
          ...
          {chartData.length < 2 ? (
            <div style={{
              ...
              <div style={{ textAlign: 'center' }}>
                Run at least 2 {disease} predictions to see your trend
              </div>
            ...
            <ResponsiveContainer width="100%" height={isMobile ? 180 : 240}>
              <AreaChart data={chartData} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
                ...
                <ReferenceLine y={50} stroke="#e5e7eb" strokeDasharray="4 4" label="" />
                <Area type="monotone" dataKey="ci_high" stroke="none" fill="url(#ciGrad)" fillOpacity={1} legendType="none" />
                <Area type="monotone" dataKey="ci_low"  stroke="none" fill="#fff"           fillOpacity={1} legendType="none" />
                <Area
                  type="monotone" dataKey="risk"
                  stroke={diseaseColors[disease]} strokeWidth={2.5}
                  dot={{ fill: diseaseColors[disease], r: 5, strokeWidth: 2, stroke: '#fff' }}
                  activeDot={{ r: 7 }} name="Risk %"
                />
              </AreaChart>
            </ResponsiveContainer>
          )}
        ...
        <div style={{
          ...
          {history.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '2rem', color: '#9ca3af', fontSize: 14 }}>
              No predictions yet.{' '}
              <span onClick={() => navigate('/predict')} style={{ color: '#6366f1', cursor: 'pointer' }}>
                Run your first one
              </span>
            </div>
          ) : isMobile ? (
